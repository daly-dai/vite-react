...
/**
 * @description 设置子路由
 * @param { object:RouteObject } moduleRouter 模块的数据
 * @param {string} path  路径
 */
const setRouterChildren = function (
  moduleRouter: RouteObject,
  path: string
): void {
  ...
  for (let i = 0, len = requireModuleList.length; i < len; i++) {
    const oRouterConfig: RouteObject = requireModule[path].default[i];

    (moduleRouter.children as RouteObject[]).push(oRouterConfig);
  }
};

/**
 * @description 根据name获取common中相应的模块数据
...
const getModuleDataByName = (key: string): any => {
  ...
  function getDataByKey(list: RouteObject[]) {
...
// // 拼接router-lazyLoad
// function assembleRouterTree(treeData) {
//   const stash = cloneDeep(treeData)
//   const result = operationAttrToNodes(stash, (node: any) => {
//     if (!isString(node['element'])) return;
//     const elementPath = matchAlias(node['element'])
//     console.log(elementPath, 78787)
//     /* @vite-ignore */
//     node['element'] = lazyLoad(lazy(() => import(elementPath)))
//   }, "children")

//   return result
// }

// function matchAlias(url) {
//   const urlStrList = url.split("/")
//   if (alias[urlStrList[0]]) {
//     urlStrList[0] = alias[urlStrList[0]];

//     return urlStrList.join("/")
//   }
